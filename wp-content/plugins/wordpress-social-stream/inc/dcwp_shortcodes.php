...
//	print_r($atts);
	...
	foreach($networks as $function=>$f){
		...
		if($function != '' && $stream['feeds_'.$function.'_id'] != '' && $stream['feeds_'.$function.'_id'] != ' '){
			...
			if($function == 'pinterest' && $p1 == 0){
				$feeds .= 'url: "'.plugins_url() . '/wordpress-social-stream/inc/dcwp_rss.php",';
			//  multisite subdomainr
			//	$feeds .= 'url: "' . get_bloginfo('url')  . '/wp-content/plugins/wordpress-social-stream/inc/dcwp_rss.php",';	
				$p1 = 1;
			}
	...
	$out .= 'if(!jQuery().dcSocialStream) { $.getScript("'.plugins_url() . '/wordpress-social-stream/js/jquery.social.stream.1.5.15.min.js", function(){$("#social-stream-'.$id.'").dcSocialStream(config);}); } else {';
...
function dc_social_feed_events_shortcode( $atts, $content = null ){
...
//	print_r($atts);
	...
	$out .= 'if(!jQuery().dcSocialStream) { $.getScript("'.plugins_url() . '/wordpress-social-stream/js/jquery.social.stream.1.5.15.min.js", function(){$("#social-stream-'.$id.'").dcSocialStream(config);}); } else {';
	$out .= '$("#social-stream-'.$id.'.dc-feed").dcSocialStream(config);}});</script>'."\n";
	$out .= '<div id="social-stream-'.$id.'" class="dc-feed '.$style.' new"></div>';
...
//	print_r($stream);
	...
	$out .='<script type="text/javascript">jQuery(document).ready(function($){';
	$out .= 'var config = '.$config.';';
	$out .= 'if(!jQuery().dcSocialStream) { $.getScript("'.plugins_url() . '/wordpress-social-stream/js/jquery.social.stream.wall.1.6.js", function(){}); $.getScript("'.plugins_url() . '/wordpress-social-stream/js/jquery.social.stream.1.5.15.min.js", function(){$("#social-stream-'.$id.'").dcSocialStream(config);}); } else {';
